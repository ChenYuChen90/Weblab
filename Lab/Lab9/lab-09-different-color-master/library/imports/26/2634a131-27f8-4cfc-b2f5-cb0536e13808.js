...
var GameManager = /** @class */ (function (_super) {
    ...
    GameManager.prototype.spawnCircle = function (row) {
        ...
        var answer = Math.floor(Math.random() * ((Math.pow(row, 2)) - 1));
        for (var i = 0; i < Math.pow(row, 2); i++) {
            var circles = cc.instantiate(this.circlePrefab);
            circles.height = circleSize;
            circles.width = circleSize;
            var btn_act = new cc.Component.EventHandler();
            btn_act.target = circles;
            btn_act.component = "Button";
            btn_act.handler = "getResult";
            if (i == answer) {
                circles.getChildByName("Background").color = this.targetColor;
                btn_act.customEventData = "Ans";
                circles.on(cc.Node.EventType.MOUSE_DOWN, function (event) {
                    _this.getResult(event, "Ans");
                }, this);
            }
            else {
                circles.getChildByName("Background").color = this.baseColor;
                btn_act.customEventData = "Wrong";
                circles.on(cc.Node.EventType.MOUSE_DOWN, function (event) {
                    _this.getResult(event, "Wrong");
                }, this);
            }
            cc.find("Canvas/circleContainer").addChild(circles);
        }
        // 
